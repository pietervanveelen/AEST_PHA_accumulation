...
### Description
This is a RMarkdown document containing the analysis of microbial communities comprising PHA accumulating bacteria on wastewater activated sludge. Input data were created by sequencing 16S rRNA gene amplicons using primers 515F and 926R on Illumina Miseq (300 bp PE). Fastq sequence files were analysed using QIIME2 and the feature table, taxonomic assignments, phylogeny and metadata were imported into R. The [MiDAS4](https://www.midasfieldguide.org/global) was used as reference data for taxonomic assignments. This analysis was published by **Estevan Alonso et al. (2022)** "url DOI" titled *Calcium addition indirectly promotes selective biomass growth and polyhydroxyalkanoate accumulation*.
...
### Data import
All input data have been created with QIIME2 and are imported in {r session_info()$platform$version}. QIIME2 scripts and parameter settings are found in separate bash files that can be found in this [Github repository](https://github.com/pietervanveelen/AEST_PHA_accumulation).<br>
...
```{r filter on abundance, message=F, echo=F, eval=T, warning=T, include=F, cache=F}

# relative abundance data
psdata_AEST_rel <- transform_sample_counts(psdata_AEST, fun = function(x) x/sum(x)) # 5154 taxa (100% abundance)

total_sum = sum(sample_sums(psdata_AEST)) # total reads left = 3684897

# abundance filter at (0.01%, 0.1% 0.5%)
psdata_AEST_0.01pct <- prune_taxa(taxa_sums(psdata_AEST_rel) > 0.0001, psdata_AEST) #3931 
psdata_AEST_0.05pct <- prune_taxa(taxa_sums(psdata_AEST_rel) > 0.0005, psdata_AEST) #2398 
psdata_AEST_0.1pct <- prune_taxa(taxa_sums(psdata_AEST_rel) > 0.001, psdata_AEST) #1849 

# taxa remaining after filters
ntaxa(psdata_AEST_0.01pct)  # 3727
ntaxa(psdata_AEST_0.05pct)  # 2252
ntaxa(psdata_AEST_0.1pct)   # 1743

# associated relative abundances with filters
100*(sum(sample_sums(psdata_AEST_0.01pct))/sum(sample_sums(psdata_AEST))) # (99.84% abundance)
100*(sum(sample_sums(psdata_AEST_0.05pct))/sum(sample_sums(psdata_AEST))) # (99.06% abundance)
100*(sum(sample_sums(psdata_AEST_0.1pct)/sum(sample_sums(psdata_AEST))))  # (98.29% abundance)
...
```{r alpha diversity , message=F, echo=F, eval=T, warning=F, include=T, cache=F, fig.height=5, fig.width=5}
...
both_mod_constrained %>% 
  kableExtra::kbl(row.names = T, align = "l", centering = T, digits = 3) %>% 
