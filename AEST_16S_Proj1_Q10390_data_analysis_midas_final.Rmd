...
```{r alpha diversity , message=F, echo=F, eval=T, warning=F, include=T, cache=F, fig.height=4, fig.width=5}
...
# using phia (F test)
phia::testInteractions(alpha_bath_mod, fixed = "time", pairwise = "condition", adjustment = "fdr")
...
# create tables
alpha_bath_mod_tab %>% 
  kableExtra::kbl(row.names = T, align = "l", centering = T, digits = 3, caption = "ANOVA Shannon diversity") %>% 
  kableExtra::kable_classic()


# plot alpha populations points
chao1_bath <- alpha_data_bath %>% 
  ggplot(aes(x=time, y=chao1, color = condition, shape = condition)) +
  #geom_boxplot(show.legend = F) + 
  scale_color_manual(values = brewer.pal(6, "Dark2")) +
  geom_jitter(position = position_dodge(0.3)) + 
  labs(y = "ASV richness (Chao1)", x= "Timepoint") +
...
shannon_bath <- alpha_data_bath %>% 
  ggplot(aes(x=time, y=shannon, color = condition, shape = condition)) +
  #geom_boxplot(show.legend = F) + 
  scale_color_manual(values = brewer.pal(6, "Dark2")) +
  geom_jitter(position = position_dodge(0.3)) + 
  labs(y = "Shannon Diversity", x= "Timepoint") +
...
# save plot
ggsave(plot = prow_bath, "figures/AEST_proj2_Alpha_diversity_BATH_Timepoint_rare69171.pdf", width = 7, height = 5)
```
